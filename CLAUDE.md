...
- **Daily orphan scan (#101):** `templates/orphan-scan-cronjob.yaml` runs `scan-orphaned` at `0 3 * * *` Europe/London (CronJob `timeZone` needs Kubernetes 1.27+), production only (`k8s.sh` sets `cronjobs.orphanScan.enabled` for the stable track; staging and canary can share production's database and would repeat the alert). Override with `ORPHAN_SCAN`, `ORPHAN_SCAN_SCHEDULE`, `ORPHAN_SCAN_TIMEZONE`. It emails `MAILER_EMAIL` (`orphaned_resources.notify.recipients`) when the orphans change. Both a bare address and the `Name <address>` form (the template default) work from bundle 2.0.0-alpha.7 (#361). The CronJob shares the API's env through the `cwa.phpEnv` helper in `_helpers.tpl`, so edit env there, not in `deployment.yaml`. **⚠ Except `RESET_DATABASE`,** which stays in `deployment.yaml` only: the entrypoint drops the schema when it's `"true"`, and the CronJob runs through the entrypoint (#103). Recipients use `%env(string:default::MAILER_EMAIL)%`: without `string:`, an empty value becomes `null` and the scan crashes (api-components-bundle#365). An unset `MAILER_DSN` renders as `null://null` (no email) instead of failing the render; GitHub workflows now pass `MAILER_DSN`, `MAILER_EMAIL` and `ORPHAN_SCAN*` (#102). Its pods are labelled `cwa-orphan-scan` so no Service selects them.
...
- **`generate_jwt_keys` (`k8s.sh`):** with no `JWT_SECRET_KEY` it generates a key pair (and a passphrase if none is set). With a key but no `JWT_PUBLIC_KEY`, it derives the public key and fails the deploy if the passphrase doesn't decrypt the key. **A key without a passphrase fails the deploy**, rather than getting a random passphrase that could never match. Use quoted POSIX `[ ]` tests on PEM values: busybox ash splits an unquoted multi-line key inside `[[ ]]` and the test silently becomes false.
