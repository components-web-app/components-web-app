...
- **`FIXTURES_PURGE` opts a fixture job into emptying the database first** (MR !11; only `load_fixtures` reads it, never a deploy or pod start, unlike `RESET_DATABASE`):
  ...
  - **⚠ Staging has no fixture job at all**, on GitLab or GitHub (Daniel, 2026-09-25). Staging's GitLab jobs run under `environment: production` and get production's `DATABASE_URL`, so a staging load would write to production. Don't add one back.
