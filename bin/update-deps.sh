...
docker-compose run app /bin/sh -c 'yarn install && yarn upgrade'
...
sed -i.bak 's/^APP_SECRET=.*$/APP_SECRET=CHANGE_ME/' .env
# Compatibility with our Docker and Kubernetes setup
sed -i.bak 's;^#TRUSTED_PROXIES=127.0.0.1,127.0.0.2$;TRUSTED_PROXIES=10.0.0.0/8,172.0.0.0/8,192.168.0.0/16;' .env
...
sed -i.bak 's;^DATABASE_URL=mysql://db_user:db_password@127.0.0.1:3306/db_name?serverVersion=5.7$;DATABASE_URL=postgres://api-platform:CHANGE_ME@db/api?server_version=12;' .env
